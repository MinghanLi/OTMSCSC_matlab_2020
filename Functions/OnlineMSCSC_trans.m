function [model,param,DeRain, Rain_Fold, Rains_Fold, Curr_Mask_Fold] = OnlineMSCSC_trans(X_Fold, param, model) 
    ...
    while ~converged && iter <= model.MaxIter
        ...
        if model.isMask == 0
            ...
            if Palpha > 0
                ...
                GCO_SetDataCost( hMRF, int32((10/Pbeta)*[ 0.5*(E_Mask(:)).^2, ~MaskOut(:)*Pbeta + MaskOut(:)*0.5*max(E_Mask(:)).^2 ]'));
%                 GCO_SetDataCost( hMRF, int32(5/std(X)*[ 5*abs(max(E(:),0)), ~OmegaOut(:)*std(X) + OmegaOut(:)*max(E(:))]'));
%                 GCO_SetDataCost( hMRF, int32(5/(std(X))^2*[ 2*(E(:)).^2, ~OmegaOut(:)*(std(X))^2 + OmegaOut(:)*max(E(:)).^2]'));
    ...
    if model.derain == 1
        DeRain = reshape(Curr_Mask.* TB + ~Curr_Mask.*F,[height,wide]);
