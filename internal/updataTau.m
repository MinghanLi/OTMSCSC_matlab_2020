function dtau =updataTau(I2, C, tau)
...
sizeD = sizeI(1)*sizeI(2);
    ...
    switch length(tau)
        case 3
            J = [(-sin(tau(1))*xCoord(:)-cos(tau(1))*yCoord(:)).*I2warp_x(:)+( cos(tau(1))*xCoord(:)-sin(tau(1))*yCoord(:)).*I2warp_y(:),I2warp_x(:), I2warp_y(:)]; % 1-by-3
        case 4
            J = [xCoord(:).*I2warp_x(:)+yCoord(:).*I2warp_y(:),...
                -yCoord(:).*I2warp_x(:)+xCoord(:).*I2warp_y(:),...
                I2warp_x(:), I2warp_y(:)]; % 1-by-4
        case 6
            J = [xCoord(:).*I2warp_x(:), xCoord(:).*I2warp_y(:),...
                 yCoord(:).*I2warp_x(:), yCoord(:).*I2warp_y(:),...
                 I2warp_x(:), I2warp_y(:)]; % 1-by-6
        case 8
            J = [ xCoord(:).*I2warp_x(:), xCoord(:).*I2warp_y(:), -(xCoord(:).^2).*I2warp_x(:)-(xCoord(:).*yCoord(:)).*I2warp_y(:),...
                  yCoord(:).*I2warp_x(:), yCoord(:).*I2warp_y(:), -(yCoord(:).^2).*I2warp_y(:)-(xCoord(:).*yCoord(:)).*I2warp_x(:),...
                  I2warp_x(:), I2warp_y(:)]; % 1-by-6
            D = [xCoord(:),yCoord(:)]*[tau(3);tau(6)]+1;
            J = bsxfun(@rdivide,J,D+eps);
        otherwise
    end    
    % solve
    temp = C(:)-I2(:);
    dtau=(J'*J+0.0001*eye(size(J,2)))\J'*temp;
